...
\title{\textbf{Propuesta de Trabajo para MINT}\\
	Análisis de la Red de Dependencias de PyPI}
...
\begin{document}
	...
	Para el trabajo final de la asignatura MINT, proponemos analizar la red de dependencias del ecosistema de paquetes de Python (PyPI). La red consistirá en un grafo dirigido donde los nodos representan paquetes de software (ej. \textit{pandas}, \textit{tensorflow}, \textit{django}) y las aristas dirigidas representan las dependencias entre ellos (un paquete requiere de otro para su instalación o funcionamiento). Se ha fijado un límite máximo de exploración de 10.000 nodos.
	
	\section{Construcción de la Red}
	La red se construirá mediante un proceso de extracción de datos automatizado (\textit{web scraping}) haciendo uso de la API pública en formato JSON de PyPI. Partiendo de una lista diversa de 10 paquetes semilla representativos de distintas áreas (ciencia de datos, desarrollo web, DevOps, etc.), se aplicará un algoritmo de búsqueda iterativo. Por cada paquete, se extraerán sus dependencias directas de forma asíncrona, añadiéndolas a la cola de procesamiento hasta alcanzar el volumen deseado para el estudio.
	...
	\begin{enumerate}
		...
		\item Analizar la distribución de grados de la red para comprobar empíricamente si sigue un modelo de red libre de escala.
