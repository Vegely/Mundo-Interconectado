...
{% open a group so arraystretch is local
	\renewcommand{\arraystretch}{1.5}
	\begin{longtable}{|c|c|p{10cm}|}
		\caption{Descripción de las comunidades...} \label{tab:communities_desc} \\
		\hline
		\textbf{ID} & \textbf{Nodos} & \textbf{Descripción} \\ \hline
		\endfirsthead  % ← ADD THIS
		...
		\hline
		\endlastfoot
		\cellcolor[RGB]{235,59,59}0 & 413 & Esta comunidad agrupa el ecosistema de computación interactiva basado en \texttt{jupyter}, \texttt{jupyterlab}, \texttt{notebook} e \texttt{ipython}, junto con su infraestructura de soporte (\texttt{ipykernel}, \texttt{jupyter-server}, \texttt{nbconvert}). Se complementa con un extenso conjunto de herramientas de documentación técnica centradas en \texttt{sphinx} y sus extensiones (\texttt{myst-parser}, \texttt{pydata-sphinx-theme}, \texttt{nbsphinx}, \texttt{sphinx-copybutton}), parsers de Markdown (\texttt{markdown-it-py}), e interfaces gráficas Qt (\texttt{PyQt5}, \texttt{qtconsole}). Incluye además herramientas de pruebas para cuadernos (\texttt{nbval}, \texttt{pytest-jupyter}) e interoperabilidad con R mediante \texttt{rpy2}. \\ \hline
		\cellcolor[RGB]{235,98,59}1 & 1197 &  Comunidad centrada en el desarrollo de APIs web y servicios backend modernos, con \texttt{fastapi}, \texttt{starlette}, \texttt{pydantic} y \texttt{uvicorn} como núcleo principal. Incluye clientes HTTP (\texttt{httpx}, \texttt{requests}, \texttt{aiohttp}), integración con servicios cloud (\texttt{boto3}, \texttt{azure-identity}, \texttt{google-cloud-aiplatform}), mensajería y colas (\texttt{celery}, \texttt{kombu}, \texttt{redis}), bases de datos (\texttt{sqlalchemy}, \texttt{pymongo}), y un extenso ecosistema de inteligencia artificial generativa con \texttt{langchain}, \texttt{openai}, \texttt{anthropic}, \texttt{llama-index} y \texttt{pydantic-ai}. También abarca herramientas de observabilidad (\texttt{opentelemetry-sdk}), autenticación (\texttt{cryptography}, \texttt{pyjwt}) y orquestación de flujos (\texttt{prefect}, \texttt{langgraph}).\\ \hline
		\cellcolor[RGB]{235,137,59}2 & 867 & Comunidad dominada por definiciones de tipos estáticos para el ecosistema AWS, con miles de paquetes \texttt{types-boto3-*} y \texttt{types-aiobotocore-*} que proveen anotaciones de tipo para cada servicio de Amazon Web Services. Complementariamente, incluye herramientas de serialización y modelado de documentación de código fuente como \texttt{databind.core}, \texttt{databind.json}, \texttt{docspec} y \texttt{docspec-python}, así como utilidades de análisis de grafos de importación (\texttt{grimp}) y logging tipado (\texttt{loguru-mypy}). Esta comunidad es esencial para el desarrollo de aplicaciones Python con tipado estático sobre infraestructura AWS. \\ \hline
		\cellcolor[RGB]{235,176,59}3 & 641 &  Comunidad orientada al análisis y manipulación de datos tabulares y geoespaciales. Su núcleo lo forman \texttt{pandas}, \texttt{numpy}, \texttt{polars}, \texttt{dask} y \texttt{xarray}, junto con conectores a bases de datos y formatos de archivo (\texttt{sqlalchemy}, \texttt{pyarrow}, \texttt{openpyxl}, \texttt{fastparquet}, \texttt{duckdb}). Incluye herramientas de visualización (\texttt{plotly}, \texttt{bokeh}, \texttt{plotnine}, \texttt{dash}), análisis geoespacial (\texttt{geopandas}, \texttt{shapely}, \texttt{fiona}, \texttt{rasterio}), astronomía (\texttt{astropy}, \texttt{skyfield}), y frameworks web ligeros para datos (\texttt{shiny}, \texttt{flask}). También abarca motores de consulta distribuida (\texttt{pyspark}, \texttt{ibis-framework}) y utilidades de grafos (\texttt{neo4j}, \texttt{networkx}).\\ \hline
		\cellcolor[RGB]{235,215,59}4 & 1128 &  Comunidad dedicada a las herramientas de calidad del software, empaquetado y desarrollo de aplicaciones Python. Agrupa frameworks de pruebas (\texttt{pytest}, \texttt{pytest-cov}, \texttt{tox}), linters y formateadores (\texttt{flake8}, \texttt{black}, \texttt{isort}, \texttt{ruff}), verificación de tipos (\texttt{mypy}, \texttt{pyright}), y generación de documentación (\texttt{sphinx}, \texttt{sphinx-rtd-theme}). Incluye herramientas de empaquetado y distribución (\texttt{setuptools}, \texttt{build}, \texttt{flit}, \texttt{hatchling}, \texttt{twine}), frameworks de aplicaciones GUI (\texttt{kivy}) y web (\texttt{django}, \texttt{flask}, \texttt{sanic}), además de utilidades de serialización (\texttt{marshmallow}), análisis estático (\texttt{pylint}, \texttt{vulture}) y herramientas de CI/CD (\texttt{dvc}, \texttt{pre-commit}).\\ \hline
		\cellcolor[RGB]{215,235,59}5 & 1169 &Gran comunidad centrada en el aprendizaje automático y la inteligencia artificial. Tiene como núcleo \texttt{tensorflow}, \texttt{torch} (PyTorch), \texttt{jax} y \texttt{keras}, junto con librerías científicas de soporte como \texttt{numpy}, \texttt{scipy}, \texttt{scikit-learn} y \texttt{matplotlib}. Abarca visión por computador (\texttt{torchvision}, \texttt{opencv-python}, \texttt{albumentations}), procesamiento de lenguaje natural (\texttt{transformers}, \texttt{tokenizers}, \texttt{sentence-transformers}, \texttt{nltk}), modelos de difusión (\texttt{diffusers}), optimización (\texttt{optuna}, \texttt{botorch}), y aceleración en GPU (\texttt{nvidia-cublas-cu12}, \texttt{triton}). También incluye herramientas de evaluación (\texttt{evaluate}, \texttt{lm-eval}), despliegue de modelos (\texttt{onnxruntime}, \texttt{vllm}) y frameworks especializados (\texttt{lightning}, \texttt{deepspeed}).  \\ \hline
		\cellcolor[RGB]{176,235,59}6 & 179 &Comunidad enfocada en computación de alto rendimiento con GPU y verificación formal de software. Agrupa librerías CUDA y RAPIDS de NVIDIA (\texttt{cudf-cu12}, \texttt{cupy-cuda12x}, \texttt{cuda-bindings}, \texttt{numba-cuda}), herramientas de pruebas basadas en propiedades (\texttt{hypothesis}, \texttt{hypothesis-crosshair}), análisis numérico (\texttt{mpmath}, \texttt{scipy-stubs}), y el ecosistema de procesamiento de lenguaje natural industrial \texttt{spacy} con sus componentes (\texttt{thinc}, \texttt{blis}, \texttt{cymem}). También incluye herramientas de la pila OpenStack (\texttt{oslo.config}, \texttt{oslo.utils}, \texttt{stevedore}) y cobertura de código (\texttt{codecov}).  \\ \hline
		\cellcolor[RGB]{137,235,59}7 & 157 &  Comunidad centrada en el ecosistema de Google Cloud Platform y sus APIs. Agrupa clientes oficiales para servicios como \texttt{google-cloud-bigquery}, \texttt{google-cloud-storage}, \texttt{google-cloud-pubsub}, \texttt{google-cloud-spanner} y \texttt{google-cloud-aiplatform}, junto con la infraestructura de autenticación y comunicación (\texttt{grpcio}, \texttt{google-auth}, \texttt{proto-plus}, \texttt{googleapis-common-protos}). Incluye además el framework de pipelines de ML \texttt{kfp} (Kubeflow Pipelines), clientes de Apache Beam para procesamiento distribuido, y herramientas criptográficas de Google (\texttt{tink}).\\ \hline
		\cellcolor[RGB]{98,235,59}8 & 2 & Comunidad mínima compuesta exclusivamente por \texttt{ansible} y \texttt{ansible-core}, representando el framework de automatización de infraestructura y gestión de configuración de sistemas. \\ \hline
		\cellcolor[RGB]{59,235,59}9 & 205 & Comunidad orientada a la documentación con \texttt{mkdocs} y sus extensiones (\texttt{mkdocs-material}, \texttt{mkdocstrings}, \texttt{mkdocs-gen-files}, \texttt{mkdocs-jupyter}), como alternativa moderna a Sphinx. Incluye el framework de arrays funcional \texttt{equinox} y \texttt{jaxtyping} para JAX, herramientas de análisis de datos con \texttt{ibis-framework} y \texttt{kedro} para pipelines de datos reproducibles, así como conectores a múltiples backends (\texttt{kedro-datasets}). También abarca formatos de datos dispersos (\texttt{sparse}, \texttt{zarr}) y herramientas de series temporales (\texttt{u8darts}). \\ \hline
		\cellcolor[RGB]{59,235,98}10 & 46 &  Comunidad centrada en el scraping web y la transformación de datos. Incluye \texttt{scrapy} como framework principal de extracción web, junto con \texttt{beautifulsoup4}, \texttt{newspaper3k} y \texttt{requests-html} para el análisis de contenido HTML. Agrupa además el ecosistema \texttt{dbt} (\texttt{dbt-core}, \texttt{dbt-postgres}, \texttt{dbt-bigquery}, \texttt{dbt-snowflake}) para transformación de datos en almacenes SQL, y herramientas de orquestación asociadas (\texttt{prefect-dbt}, \texttt{prefect-sqlalchemy}).\\ \hline
		\cellcolor[RGB]{59,235,137}11 & 115 &  Comunidad agrupada en torno a \texttt{ruamel.yaml} y sus extensiones para el procesamiento avanzado de YAML, junto con servidores web de alto rendimiento como \texttt{granian} y \texttt{uvloop}. Incluye el framework de MLOps \texttt{mlrun} con sus numerosas dependencias versionadas, herramientas de validación de especificaciones OpenAPI (\texttt{prance}, \texttt{openapi-spec-validator}), y clientes para servicios cloud de Alibaba (\texttt{aliyun-python-sdk-core}, \texttt{oss2}). También contiene \texttt{deepdiff} para comparación de estructuras de datos.\\ \hline
		\cellcolor[RGB]{59,235,176}12 & 5 &  Comunidad mínima formada por el binding oficial de Qt para Python mediante \texttt{PySide6}, \texttt{shiboken6}, \texttt{PySide6\_Essentials} y \texttt{PySide6\_Addons}, representando la alternativa LGPL a PyQt5 para el desarrollo de interfaces gráficas de escritorio con Qt6.\\ \hline
		\cellcolor[RGB]{59,235,215}13 & 28 & Comunidad centrada en el análisis sintáctico y la representación estructurada de texto. Agrupa \texttt{tree-sitter} y sus gramáticas para múltiples lenguajes (\texttt{tree-sitter-python}, \texttt{tree-sitter-javascript}, \texttt{tree-sitter-rust}, entre otros), utilizados para el análisis de código fuente. Incluye además \texttt{textual} para interfaces de terminal, \texttt{docling-core} y \texttt{docling-parse} para extracción de documentos, y \texttt{latex2mathml} para conversión de fórmulas matemáticas. \\ \hline
		\cellcolor[RGB]{59,215,235}14 & 246 & Comunidad dominada por el AWS Cloud Development Kit (\texttt{aws-cdk-lib} y la familia \texttt{aws-cdk.aws-*}), que proporciona abstracciones de infraestructura como código para prácticamente todos los servicios de Amazon Web Services. El núcleo lo forman \texttt{constructs}, \texttt{jsii} y \texttt{aws-cdk.core}, con módulos especializados para servicios como \texttt{aws-cdk.aws-lambda}, \texttt{aws-cdk.aws-ec2}, \texttt{aws-cdk.aws-s3}, \texttt{aws-cdk.aws-rds} y \texttt{aws-cdk.aws-eks}, entre muchos otros. \\ \hline
		\cellcolor[RGB]{59,176,235}15 & 220 &  Comunidad centrada en el framework de red asíncrono \texttt{twisted} y el extenso ecosistema de bindings \texttt{pyobjc} para macOS. El componente \texttt{twisted} incluye soporte para protocolos de red, TLS y concurrencia (\texttt{automat}, \texttt{hyperlink}, \texttt{incremental}), mientras que \texttt{pyobjc} expone prácticamente todos los frameworks nativos de Apple (\texttt{pyobjc-framework-Cocoa}, \texttt{pyobjc-framework-Metal}, \texttt{pyobjc-framework-CoreML}, entre otros). También incluye \texttt{rinohtype} para composición tipográfica de documentos PDF y \texttt{mido} para el manejo de datos MIDI.\\ \hline
		\cellcolor[RGB]{59,137,235}16 & 4 &  Comunidad mínima formada por \texttt{Pyro4} y sus dependencias directas (\texttt{serpent}, \texttt{selectors34}), que implementan un sistema de objetos remotos (\textit{Remote Object System}) para Python, permitiendo la comunicación transparente entre procesos distribuidos.\\ \hline
		\cellcolor[RGB]{59,98,235}17 & 5 &  Comunidad pequeña agrupada en torno a utilidades de logging con color y formato legible para la terminal, representada por \texttt{coloredlogs} y \texttt{humanfriendly}, junto con \texttt{capturer} para captura de salida estándar, y compatibilidades de bajo nivel (\texttt{monotonic}, \texttt{pyreadline}).\\ \hline
		\cellcolor[RGB]{59,59,235}18 & 64 &  Comunidad formada íntegramente por el framework multiagente \texttt{ag2} (evolución de AutoGen) y sus numerosas variantes de instalación opcionales (\texttt{ag2[openai]}, \texttt{ag2[anthropic]}, \texttt{ag2[langchain]}, \texttt{ag2[rag]}, \texttt{ag2[browser-use]}, entre otras), que cubren integraciones con distintos proveedores de modelos de lenguaje, bases de datos vectoriales, herramientas de búsqueda y capacidades multimodales para la construcción de sistemas de agentes autónomos.\\ \hline
		\cellcolor[RGB]{98,59,235}19 & 4 &  Comunidad mínima compuesta por \texttt{mitmproxy-rs} y sus variantes específicas por plataforma (\texttt{mitmproxy-windows}, \texttt{mitmproxy-linux}, \texttt{mitmproxy-macos}), que proporcionan los componentes nativos en Rust para el proxy HTTP/HTTPS interactivo \texttt{mitmproxy}.\\ \hline
		\cellcolor[RGB]{137,59,235}20 & 4 &  Comunidad mínima formada por el binding de Qt6 para Python \texttt{PyQt6} y sus componentes asociados (\texttt{PyQt6-sip}, \texttt{PyQt6-Qt6}), representando la versión más reciente del framework para el desarrollo de interfaces gráficas de escritorio multiplataforma con PyQt.\\ \hline
		\cellcolor[RGB]{176,59,235}21 & 18 & Comunidad compuesta por librerías de bajo nivel de NVIDIA para computación en GPU con CUDA 13, incluyendo componentes como \texttt{nvidia-cublas}, \texttt{nvidia-cudnn-cu13}, \texttt{nvidia-cuda-runtime}, \texttt{nvidia-nccl-cu13} y \texttt{nvidia-nvshmem-cu13}, junto con el plugin \texttt{jax-cuda13-plugin} para la integración de JAX con esta generación de CUDA. \\ \hline
		\cellcolor[RGB]{215,59,235}22 & 14 &  Comunidad formada por librerías de bajo nivel del ecosistema ROCm de AMD para computación en GPU, incluyendo \texttt{amd\_rocm\_hip\_runtime\_instinct}, \texttt{amd\_hipblas\_instinct}, \texttt{amd\_miopen\_hip\_instinct} y \texttt{amd\_rccl\_instinct}, junto con el plugin \texttt{jax-rocm7-plugin} para la integración de JAX con hardware GPU de AMD.\\ \hline
		\cellcolor[RGB]{235,59,215}23 & 9 & Comunidad pequeña formada por el SDK nativo de Python para Amazon Bedrock (\texttt{aws-sdk-bedrock-runtime}) y la pila de librerías Smithy (\texttt{smithy-core}, \texttt{smithy-http}, \texttt{smithy-aws-core}, \texttt{smithy-json}), que implementan el framework de modelado de servicios de AWS para la generación y consumo de APIs cloud de nueva generación. \\ \hline
		\cellcolor[RGB]{235,59,176}24 & 10 & Comunidad centrada en el desarrollo de interfaces de usuario interactivas en la terminal. Agrupa \texttt{inquirer} para menús de selección interactivos, \texttt{blessed} y \texttt{jinxed} para el control de terminal multiplataforma, \texttt{readchar} para la lectura de teclas, y extensiones de Sphinx como \texttt{sphinx-paramlinks} y \texttt{sphinxcontrib-manpage} para documentación de parámetros y páginas de manual. \\ \hline
		\cellcolor[RGB]{235,59,137}25 & 7 &  Comunidad mínima formada por el ecosistema \texttt{itkwasm} para el procesamiento de imágenes médicas en WebAssembly, incluyendo \texttt{itkwasm-image-io}, \texttt{itkwasm-downsample} y sus variantes para los entornos de ejecución Emscripten y WASI, que permiten ejecutar algoritmos de la librería ITK directamente en el navegador o en entornos WASM.\\ \hline
		\cellcolor[RGB]{235,59,98}26 & 8 & Comunidad mínima centrada en la verificación matemática formal, compuesta por \texttt{math-verify} para la comprobación automática de soluciones matemáticas y \texttt{latex2sympy2\_extended} para la conversión de expresiones \LaTeX{} a objetos simbólicos de SymPy, con variantes para distintas versiones de ANTLR4 (\texttt{antlr4\_9\_3}, \texttt{antlr4\_11\_0}, \texttt{antlr4\_13\_2}). Se utiliza principalmente en el contexto de evaluación de modelos de lenguaje en tareas matemáticas (\texttt{lighteval[math]}). \\ \hline
	\end{longtable}
}% close the group
