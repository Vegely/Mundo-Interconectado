...
\begin{document}
	...
	De entre todos los nodos, un total de \(6763 (99.97\%)\) pertenecen a la componente gigante. Los paquetes \texttt{ansible} y \texttt{ansible-core\textasciitilde{}} pertenecen a una isla desconectada del resto del grafo. Resulta extraño que \texttt{ansible}, un paquete de automatización ampliamente utilizado en la industria para configurar servidores y servicios por lotes, no tenga dependencias de otros paquetes. Una posible explicación se encuentra en la forma de construir la red. Al fijar un límite máximo de 10000 nodos visitados\footnote{Nótese que la red tiene 6765 nodos, por lo que el contador considera todas las visitas independientemente de que sean al mismo nodo.}, podría ocurrir que \texttt{ansible-cores\textasciitilde{}} hubiera sido la última y no se analizaran sus dependencias. Las figuras \ref{fig:conectedcomponent} y \ref{fig:conectedin} representan la componente gigante.
	...
	\begin{table}[htpb]
		...
		\begin{tabular}{lrcl}
			...
			\midrule
			\multicolumn{4}{c}{\textbf{Grado de Entrada (IN)} \quad ($k_{\min}=3$, $\alpha=1.900$, $p_{\text{KS}}=0.532$)} \\
			\midrule
			Exponencial & $8.432$ & $\mathbf{<0.001}$ & Ley de potencias favorable \\
			Poisson     & $5.401$ & $\mathbf{<0.001}$ & Ley de potencias favorable \\
			Yule        & $-1.598$ & $0.110$ & No distinguibles \\
			\midrule
			\multicolumn{4}{c}{\textbf{Grado de Salida (OUT)} \quad ($k_{\min}=11$, $\alpha=2.516$, $p_{\text{KS}}<0.001$)} \\
			\midrule
			Exponencial & $1.307$ & $0.191$ & No distinguibles \\
			Poisson     & $3.824$ & $\mathbf{<0.001}$ & Ley de potencias favorable \\
			Yule        & $-8.943$ & $\mathbf{<0.001}$ & \textbf{Alternativa favorable} \\
			\bottomrule
		\end{tabular}
	...
	\subsection{PageRank}
	Como se trabaja con un grafo dirigido, esta medida de centralidad es de aplicación directa al análisis de trabajo. Como el PageRank se puede entender como una medida de cual es la probabilidad de acabar en un paquete al realizar un camino aleatorio markoviano sobre el grafo con saltos aleatorios con una probabilidad $\theta=0.85$. Por tanto, puede entenderse que 
